      ...
      <div className="max-w-7xl mx-auto">
                ...
                <Link to={section.link}>
                  <Card className={`group hover:shadow-2xl transition-all duration-300 border-0 bg-gradient-to-br ${section.color} overflow-hidden h-full hover:-translate-y-1 cursor-pointer rounded-3xl`}>
                    <CardContent className="p-4 md:p-6 text-center">
                      <div className="w-12 h-12 md:w-16 md:h-16 rounded-full bg-white dark:bg-slate-800/90 dark:bg-slate-700/90 backdrop-blur-sm flex items-center justify-center mb-3 mx-auto group-hover:scale-110 transition-transform duration-300 shadow-lg overflow-hidden transition-colors duration-300">
                        <img src={section.image} alt={section.title} className="w-8 h-8 md:w-12 md:h-12 object-contain" />
                      </div>
                      <h3 className="text-sm md:text-base font-bold text-gray-800 dark:text-white mb-1 transition-colors duration-300">{section.title}</h3>
                      <p className="text-xs text-gray-700 dark:text-gray-300 leading-relaxed transition-colors duration-300">{section.description}</p>
                    </CardContent>
                  </Card>
                </Link>
              ) : (
        ...
        <div id="principles" className="space-y-6 mb-8">
          {principles.map((principle, index) => (
            ...
            >
              <Card className="border-0 shadow-2xl bg-white dark:bg-slate-800/95 dark:bg-slate-800/50 backdrop-blur-sm rounded-3xl overflow-hidden transition-colors duration-300 transition-colors duration-300">
                <CardHeader className="bg-gradient-to-r from-emerald-500 to-emerald-600 dark:from-teal-600 dark:to-teal-700 text-white p-6 md:p-8 transition-colors duration-300">
                  <CardTitle className="text-xl md:text-2xl">{principle.title}</CardTitle>
                </CardHeader>
                <CardContent className="p-6 md:p-8">
                  <div className="space-y-4">
                    {principle.items.map((item, itemIndex) => (
                      <div key={itemIndex} className="flex gap-4 items-start bg-gradient-to-br from-emerald-50 to-teal-50 dark:from-slate-700/50 dark:to-slate-600/50 p-4 md:p-5 rounded-2xl shadow-md transition-colors duration-300 transition-colors duration-300">
                        <div className="w-10 h-10 rounded-full bg-gradient-to-br from-emerald-400 to-emerald-600 dark:from-teal-500 dark:to-teal-700 flex items-center justify-center flex-shrink-0 shadow-md transition-colors duration-300">
                          <span className="text-white font-bold">{itemIndex + 1}</span>
                        </div>
                        <p className="text-gray-700 dark:text-gray-200 text-sm md:text-base leading-relaxed pt-2 transition-colors duration-300">{item}</p>
                      </div>
                    ))}
                  </div>
                </CardContent>
        ...
        >
          <Card className="border-0 shadow-2xl bg-gradient-to-br from-emerald-500 to-emerald-600 dark:from-slate-800/80 dark:to-slate-700/80 text-white overflow-hidden relative rounded-3xl transition-colors duration-300 transition-colors duration-300">
