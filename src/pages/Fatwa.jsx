...
export default function Fatwa() {
  ...
  if (selectedFatwa) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-emerald-700 via-emerald-600 to-emerald-800 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 p-4 md:p-6 transition-colors duration-300">
        <div className="max-w-4xl mx-auto">
          <Button
            onClick={() => setSelectedFatwa(null)}
            variant="ghost"
            className="text-white hover:bg-white/20 dark:hover:bg-slate-800/50 mb-6"
          >
            <ArrowRight className="w-5 h-5 ml-2" />
            رجوع للفتاوى
          </Button>

          <Card className="border-0 shadow-2xl bg-white/95 dark:bg-slate-800/50 backdrop-blur-sm rounded-3xl mb-6 transition-colors duration-300">
            <CardHeader className="bg-gradient-to-r from-emerald-500 to-emerald-600 dark:from-emerald-600 dark:to-emerald-700 text-white rounded-t-3xl p-6 md:p-8">
              <CardTitle className="text-xl md:text-2xl">{selectedFatwa.question}</CardTitle>
            </CardHeader>
            <CardContent className="p-6 md:p-8 space-y-6">
              <div>
                <div className="flex items-center justify-between mb-3">
                    <h3 className="text-lg font-bold text-gray-900 dark:text-white">الجواب:</h3>
                    <Button 
                        size="sm" 
                        variant="ghost" 
                        className="text-purple-600 dark:text-purple-400 hover:bg-purple-50 dark:hover:bg-slate-700 gap-1"
                        onClick={async () => {
                            try {
                ...
                <p className="text-gray-700 dark:text-gray-300 leading-relaxed whitespace-pre-wrap">{selectedFatwa.answer}</p>
              </div>
              ...
              {selectedFatwa.reference && (
                <div className="p-4 bg-amber-50 dark:bg-slate-700/50 rounded-lg border border-amber-200 dark:border-slate-600 transition-colors duration-300">
              ...
              <div className="flex items-center justify-between">
                ...
                <Button 
                   variant="outline" 
                   className="gap-2 text-rose-600 dark:text-rose-400 hover:text-rose-700 hover:bg-rose-50 dark:hover:bg-slate-700"
                   onClick={async () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-emerald-700 via-emerald-600 to-emerald-800 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 p-4 md:p-6 transition-colors duration-300">
      <div className="max-w-7xl mx-auto">
        ...
        >
          <div className="inline-flex items-center gap-2 bg-gradient-to-r from-emerald-100 to-teal-100 dark:from-slate-800/80 dark:to-slate-700/80 px-6 py-3 rounded-full mb-6 transition-colors duration-300">
            <BookOpen className="w-5 h-5 text-emerald-600 dark:text-emerald-400" />
            <span className="text-emerald-800 dark:text-emerald-300 font-semibold">الفتاوى الشرعية</span>
          </div>
            ...
            >
              ...
              >
                ...
                <CardContent className="p-6 md:p-8 text-center">
                  ...
                  <h3 className="text-base md:text-lg font-bold text-gray-800 dark:text-white mb-2">{action.title}</h3>
                  <p className="text-sm text-gray-700 dark:text-gray-300 leading-relaxed">{action.description}</p>
                </CardContent>
              </Card>
            </motion.div>
          ))}
        ...
        <Card className="border-0 shadow-2xl bg-white/95 dark:bg-slate-800/50 backdrop-blur-sm rounded-3xl mb-8 transition-colors duration-300">
            ...
            <div className="space-y-4">
              <div className="relative">
                ...
                <Input
                  ...
                  className="pr-12 py-6 text-lg bg-white dark:bg-slate-700 dark:text-white dark:placeholder-gray-400 rounded-full border-2 border-gray-200 dark:border-slate-600 focus:border-emerald-500 dark:focus:border-emerald-400 transition-colors duration-300"
                />
              </div>
              ...
              <Button
                ...
                className="text-emerald-600 dark:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-slate-700"
              >
              ...
              {showAdvancedSearch && (
                <motion.div
                  ...
                  className="grid md:grid-cols-3 gap-4 pt-4 border-t dark:border-slate-600"
                >
                  <Input
                    placeholder="اسم المفتي"
                    value={muftiQuery}
                    onChange={(e) => setMuftiQuery(e.target.value)}
                    className="dark:bg-slate-700 dark:text-white dark:placeholder-gray-400"
                  />
                  <Input
                    type="date"
        ...
        {filteredFatwas.length === 0 && !isLoading && (
          <Card className="border-0 shadow-2xl bg-white/95 dark:bg-slate-800/50 backdrop-blur-sm rounded-3xl p-12 text-center transition-colors duration-300">
            <BookOpen className="w-16 h-16 text-gray-400 dark:text-gray-500 mx-auto mb-4" />
            <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">لا توجد نتائج</h3>
            <p className="text-gray-600 dark:text-gray-400">جرب البحث بكلمات مختلفة</p>
          </Card>
        )}
      </div>
